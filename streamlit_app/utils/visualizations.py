...
def create_attack_distribution_pie(results: list):
    ...
    # Color mapping: green = normal, orange = anomaly/uncertain, red = confirmed attack
    def get_color(label):
        if label == 'Normal Traffic':
            return '#2ecc71'
        elif label in ['Anomaly - Needs Review', 'Uncertain - Needs Review']:
            return '#e67e22'
        else:
            return '#e74c3c'

    colors = [get_color(k) for k in attack_types.keys()]
...
def create_attack_types_bar(results: list, top_n: int = 10):
    ...
    for result in results:
        if isinstance(result, dict):
            ...
            if classification and classification not in ['Normal Traffic', 'Anomaly - Needs Review', 'Uncertain - Needs Review']:
